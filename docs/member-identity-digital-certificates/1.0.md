...
Client and signing certificates include the Directory URL of the Member in the Subject of the certificate, along with the Roles of the Member and an Application identifier to indicate the purpose of their use of data.
 ...
 * the Member's Directory URL, which identifies them in all metadata
 ...
 * CN=[Trust Framework] Client CA
    * CN=[Trust Framework] Client Issuer
        * C=[Country code] O=[Client Name] CN=[Member Directory URL], subjectAlternativeName=URI:[Member Directory URL]

See below for the x509 extensions that are required for the certificates.

### Obtaining a client certificate

A member logs into the Directory, or uses an API, provides a CSR, and requests a client certificate. The Directory generates a certificate which encodes the Member's identity and Roles, ignoring everything other than the public key in the CSR.
...
A member logs into the Directory, or uses an API, provides a CSR, and requests a signing certificate. The Directory generates a certificate which encodes the Member's identity and Roles, ignoring everything other than the public key in the CSR..
...
`ib1Application` (`1.3.6.1.4.1.62329.1.2`)
