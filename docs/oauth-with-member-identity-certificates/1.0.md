...
All server-to-server communication is secured with mTLS using client certificates. In the certificate, the client is identified by a URL in the client X.509 certificate as the single URI type Subject Alternative Name.
