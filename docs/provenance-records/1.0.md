...
Steps are serialised as JSON, then Base64 encoded for inclusion in a JSON encoded container. The structure and cryptographic choices borrow from [JWS](https://www.rfc-editor.org/rfc/rfc7515), with adaptations for nested signatures and use of x509 certificates to provide keys and identify participants.
